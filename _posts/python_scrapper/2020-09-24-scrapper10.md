...
<html>
    ...
    <body>
        ...
        <section>   
            ...
            <h4>location</h4>
            <h4>link</h4>
             {% for job in jobs %} <!-- for loop 함수를 html 에서 돌리려면 {%%} 를 추가하면 끝! report함수에서 rendering 할때 jobs = jobs[:10] 를 추가했는데 여기 for loop에 사용되는 것이다.-->
               <span>{{job.Title}}</span> <!-- 여기서도 rendering을 쓴다({{}}) -->
